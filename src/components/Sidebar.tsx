...
function QuickActionButton({ icon: Icon, label, onClick, isActive = false }: QuickActionButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left text-sm transition-colors cursor-pointer ${isActive ? "bg-foreground text-background" : "text-foreground hover:bg-background"}`}
    >
      ...
      >
        <div className="flex items-center justify-between border-b border-(--border) px-4 py-3">
          <div className="flex items-center gap-3">
            <div
              className="flex h-10 w-10 items-center justify-center rounded-2xl border border-(--border) bg-background"
              style={{ boxShadow: "var(--shadow-sm)" }}
            >
              <RaavanMark className="h-6 w-6" />
            </div>
            <div className="text-sm font-semibold text-foreground">Raavan</div>
          </div>
          {onCollapse && (
            <button
              type="button"
              ...
              className="flex h-9 w-9 items-center justify-center rounded-xl border border-transparent text-(--muted) transition-colors hover:border-(--border) hover:bg-background hover:text-foreground cursor-pointer"
            >
              <SidebarToggleIcon direction="close" className="h-4 w-4" />
            </button>
          )}
        </div>
        ...
        {isSettingsMode ? (
          ...
          <>
            <div className="px-3 pb-2 pt-3">
              <div className="flex items-center gap-2 rounded-2xl border border-(--border) bg-background px-3 py-2.5 text-sm text-(--muted)">
                ...
                {search && (
                  <button
                    type="button"
                    onClick={() => setSearch("")}
                    className="cursor-pointer rounded-lg p-1 hover:bg-(--card-hover)"
                  >
                    <X className="h-3 w-3" />
                  </button>
                )}
                ...
                Object.entries(threadGroups).map(([label, items]) => {
                  ...
                  return (
                    <div key={label} className="mb-4">
                      <p className="px-3 pb-1.5 pt-2 text-[11px] font-medium uppercase tracking-[0.12em] text-(--muted)">
                        {label}
                      </p>
...
function ThreadItem({
  ...
  return (
    <div
      className={`group relative flex cursor-pointer items-center rounded-xl transition-colors ${isActive ? "bg-(--card) text-foreground" : "text-(--muted) hover:bg-(--card) hover:text-foreground"}`}
      onClick={onSelect}
      style={isActive ? { boxShadow: "var(--shadow-sm)" } : undefined}
    >
      <div className="min-w-0 flex-1 px-3 py-2.5">
        <p className={`truncate text-[13px] leading-snug ${isActive ? "font-medium" : ""}`}>{thread.name}</p>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-0.5 pr-1.5 opacity-0 transition-opacity group-hover:opacity-100">
        ...
        <button
          ...
          className="cursor-pointer rounded-lg p-1 text-red-400 hover:bg-red-500/15"
          title="Delete"
        >
